...
> Source: [documentation/90-FAQ.md.html](https://awesomewm.org/doc/api/documentation/90-FAQ.md.html)
...
The name _Awesome_ comes from the English word _awesome_ often used by the character [Barney Stinson](<https://en.wikipedia.org/wiki/Barney_Stinson>) from the TV series HIMYM.

## Common issues

### My screens are not configured correctly

Awesome is a window manager, not a desktop environment. To configure your screens, the `xrandr` tool or various GUI applications exist. For more information, refer to the [Arch Linux wiki](<https://wiki.archlinux.org/index.php/xrandr>).

### There is a noticeable delay with inputs

There are two common causes for this:

The first is a display driver issue where painting on the screen takes long time. All input events (keyboard and mouse) are processed in the main thread. The drawing is also locking the main thread to avoid artifacts and race ccontiditions. If there is a delay with the painting, it will delay the inputs. The solution to this problem is using a compositing manager such as [compton](<https://github.com/chjj/compton>) or the older `xcompmgr`. This will move painting to another process and fully mitigate the issue.

The second one is when using [io.popen](<https://www.lua.org/manual/5.1/manual.html#pdf-io.popen>) or other blocking functions in [rc.lua](<../sample files/rc.lua.html#>) and most commonly manifests itself as occasional freezes instead of a generic delay. Do **not** use such functions and prefer [awful.spawn.easy_async](<../libraries/awful.spawn.html#easy_async>), [awful.widget.watch](<../classes/awful.widget.watch.html#>) or the GIO async API. Even if you _think_ a command is fast enough and won’t impact the main event loop iteration time, you are wrong. _Every_ calls to [io.open](<https://www.lua.org/manual/5.1/manual.html#pdf-io.open>) are impacted by the system `iowait` queue and can spend hundreds of milliseconds blocked _before_ being executed. Note that some common widget or probe libraries such as [Vicious](<https://github.com/Mic92/vicious>) do not follow this advice currently and are known to cause input lag on some systems (but not all).
...
Awesome is a window manager, not a desktop environment. It does not provide a theme daemon. For more information about how to manage the look and feel of applications, refer to the [Arch Linux Wiki](<https://wiki.archlinux.org/index.php/Category:Eye_candy>).

## Configuration

### How to change the default window management layout?

In the default configuration file one layout is set for all tags, it happens to be the `floating` layout. You can change that by editing your tag creation loop in the [rc.lua](<../sample files/rc.lua.html#>):
...
At the beginning of [rc.lua](<../sample files/rc.lua.html#>):
...
If you want to execute a shell command or need to execute a command that uses redirection, pipes and so on, do not use the [awful.spawn](<../libraries/awful.spawn.html#>) function but [awful.spawn.with_shell](<../libraries/awful.spawn.html#with_shell>). Here is an example:
...
You can add `size_hints_honor = false` to the `properties` section in your [awful.rules.rules](<../libraries/awful.rules.html#rules>) table in your [rc.lua](<../sample files/rc.lua.html#>). It will match and apply this rule to all clients.

See [the mailing list archive](<http://www.mail-archive.com/awesome@naquadah.org/msg01767.html>) for more info about what size hints are.
...
To disable titlebars on all clients remove the `titlebars_enabled=true` from the [awful.rules.rules](<../libraries/awful.rules.html#rules>) table in your config. If you want a titlebar only on certain clients, you can use [awful.rules](<../libraries/awful.rules.html#>) to set this property only for certain clients.
...
The default [rc.lua](<../sample files/rc.lua.html#>) already has a key binding for this, it is “Mod4 + Control + Space”. You can easily change it to something easier like “Mod4 + f” or “Mod4 + Shift + f”.
...
You can ensure no application ever starts maximized in the first rule of your [awful.rules.rules](<../libraries/awful.rules.html#rules>) table, which applies to all clients, by adding:
...
You can use the [client:relative_move](<../classes/client.html#client:relative_move>) function. The following `clientkeys` example will move floaters with “Mod4 + Arrow keys” and resize them with “Mod4 + PgUP/DN” keys:
...
You can modify [rc.lua](<../sample files/rc.lua.html#>), but you have to restart Awesome for changes to take effect. The default keybinding for restarting Awesome is “Mod4 + Control + r”.
...
You can use the above identifiers (instance, class and name) in your [awful.rules.rules](<../libraries/awful.rules.html#rules>) table to do matching, tagging and other client manipulation. See the next FAQ answer for some examples.

### How to start clients on specific tags and others as floating?

You can add matching rules to your [awful.rules.rules](<../libraries/awful.rules.html#rules>) table. The default [rc.lua](<../sample files/rc.lua.html#>) already has several examples, but some more can be found in the [documentation](<../libraries/awful.rules.html#rules>).
...
The `wibox.widget.keyboardlayout` is a widget that shows the current keyboard layout and allows to change it by clicking on it.

### How to make windows spawn under the mouse cursor?

In the default [awful.rules](<../libraries/awful.rules.html#>)-rule, the following placement is specified:
    
    
    placement = awful.placement.no_overlap+awful.placement.no_offscreen
    

You can prepend [awful.placement.under_mouse](<../libraries/awful.placement.html#under_mouse>) to this:
...
You can change this by redefining `awful.ewmh.activate©` in your rc.lua. If you don’t want new clients to be urgent by default put this in your rc.lua:
...
You can use the keybinding “Mod4+Ctrl+r” or by selecting restart in the menu. You could call [awesome.restart](<../libraries/awesome.html#restart>) either from the Lua prompt widget, or via `awesome-client`:
...
You can quit Awesome by using “Mod4+Shift+q” keybinding or by selecting quit in the menu. You could call [awesome.quit](<../libraries/awesome.html#quit>) either from the Lua prompt widget, or by passing it to `awesome-client`.
...
If Awesome cannot find `$XDG_CONFIG_HOME/awesome/rc.lua`, or fails to load it, it falls back to using `/etc/xdg/awesome/rc.lua` (you haven’t edited it, I hope, have you?). Even if `awesome –check` hasn’t reported any error, it only means that your [rc.lua](<../sample files/rc.lua.html#>) is syntactically correct, but absence of runtime errors is not guaranteed. Moreover, Awesome could apply half of your config then encounter an error and load stock one, and that could lead to bizarre result, like two sets of tags. See the next entry on how to find out where the problem lurks.
...
This avoids losing your tags, layout and focus history. Olders [rc.lua](<../sample files/rc.lua.html#>) were not designed to support such changes and assumed Awesome would restart. To add multi-screen support to existing configs, see how `awful.screen.connect_for_each_screen` is used in the new [rc.lua](<../sample files/rc.lua.html#>) or rebuild your config on a newer revision of [rc.lua](<../sample files/rc.lua.html#>).
...
No. See the previous two questions. However, it is possible to swap tags across screens using `t:swap(t2)` (assuming `t` and `t2` are valid [tag](<../classes/tag.html#>) objects).
...
Yes, we do. You can submit pull requests on the [github repository](<https://github.com/awesomeWM/awesome>). Please read the [contributing guide](<https://github.com/awesomeWM/awesome/blob/master/docs/02-contributing.md>) for any coding, documentation or patch guidelines.